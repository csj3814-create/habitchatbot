 ...
 *
 * [방 구분 없음 — 알고 쓰세요]
 * 이 스크립트는 방을 가리지 않습니다. '!'로 시작하는 메세지는 이 폰의 카카오톡이
 * 들어가 있는 '모든 방'에서 서버로 전달됩니다.
 * 메신저봇R v0.7.29a가 넘겨주는 room 값은 방 이름이 아니라 알림 제목(= 말한 사람의
 * 대화명)이라 방을 식별할 수 없고, isGroupChat도 오픈채팅에서 false로 옵니다.
 * 이 버전에는 BotManager/Event API가 없어 channelId도 없습니다.
 * 따라서 방 분리는 코드가 아니라 운영 약속입니다: 해빛스쿨은 '!', 해피닥터는 '~'.
 *
