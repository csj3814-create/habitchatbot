...
app.post('/api/chat', async (req, res) => {
    ...
    // !안내 — 해빛스쿨 상세 가이드
    if (actualQuestion === '안내' || actualQuestion === '시작' || actualQuestion === '가이드') {
        const guideMsg = await handleGuide(userName);
        return res.status(200).json({
            version: "2.0",
            template: { outputs: [{ simpleText: { text: guideMsg } }] }
        });
    }
...
app.post('/api/messengerbot', async (req, res) => {
    ...
    try {
        ...
        if (command === '안내' || command === '시작' || command === '가이드') {
            const reply = await handleGuide(sender);
            return res.status(200).json({ reply });
        }
        ...
        try {
            ...
            if (mapping) {
                ...
                if (recentRecords.length > 0) {
                    ...
                    // 기록 일수
                    const dietDays = recentRecords.filter(r => r.diet && (r.diet.breakfastUrl || r.diet.lunchUrl || r.diet.dinnerUrl)).length;
                    const exDays = recentRecords.filter(r => r.exercise && ((r.exercise.cardioList?.length > 0) || (r.exercise.strengthList?.length > 0))).length;
                    const mindDays = recentRecords.filter(r => r.sleepAndMind && (r.sleepAndMind.sleepImageUrl || r.sleepAndMind.meditationDone || r.sleepAndMind.gratitude)).length;
