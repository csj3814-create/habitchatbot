...
// 앱 연동 모듈
const { getUserRecords, getGalleryByDate } = require('./modules/appFirebase');
...
app.post('/api/messengerbot', async (req, res) => {
    ...
    try {
        ...
        if (command === '랭킹' || command === '순위') {
            const reply = await handleRanking();
            return res.status(200).json({ reply });
        }
...
// ===== 브로드캐스트 엔드포인트 (메신저봇R schedule() 함수에서 호출) =====
// 하루 4회: 아침(08:00) / 점심(12:00) / 저녁(18:30) / 취침전(21:00)
...
// 저녁 브로드캐스트: 오늘 현황 + 격려
async function buildNightBroadcast() {
    const { hasDiet: hd, hasExercise: he, hasMind: hm } = require('./modules/statsHelpers');
    ...
    const dietCount = logs.filter(hd).length;
    const exerciseCount = logs.filter(he).length;
    const mindCount = logs.filter(hm).length;
...
app.get('/api/broadcast/morning', (req, res) => {
    const quote = MORNING_QUOTES[new Date().getDay() % MORNING_QUOTES.length];
    const dow = ['일', '월', '화', '수', '목', '금', '토'][new Date(getKstDateStr() + 'T00:00:00+09:00').getDay()];
    const msg = `🌞 좋은 아침이에요! (${dow}요일)\n━━━━━━━━━━━━━━━\n${quote}\n\n📱 아침 식단 기록하러 가기\n👉 해빛스쿨 앱 열기\n\n!우리반 으로 오늘 현황 확인!`;
    res.json({ message: msg });
});

// GET /api/broadcast/lunch — 점심 식사 독려 (12:00)
app.get('/api/broadcast/lunch', (req, res) => {
    const quote = LUNCH_QUOTES[new Date().getDay() % LUNCH_QUOTES.length];
    const msg = `🍱 점심 시간이에요!\n━━━━━━━━━━━━━━━\n${quote}\n\n📸 지금 바로 앱에서 점심 기록!\n오늘 !오늘 로 친구들 현황도 확인해요 👀`;
    res.json({ message: msg });
});

// GET /api/broadcast/dinner — 저녁 식사 + 운동 독려 (18:30)
app.get('/api/broadcast/dinner', (req, res) => {
...
app.get('/api/broadcast/night', async (req, res) => {
    try {
        const msg = await buildNightBroadcast();
        res.json({ message: msg });
    } catch (e) {
